...
# ---------- GERENCIADOR COMPARTILHADO PARA ESTATÍSTICAS ----------
manager = mp.Manager()

# Estatísticas dos workers (já estava)
worker_stats = manager.dict()
worker_stats_lock = mp.Lock()

# Estatísticas agregadas (agora compartilhadas)
stats = manager.dict()
stats_lock = mp.Lock()
...
def mix_and_export_task(segments_data, ambient_cfg, target_rate=22050):
    ...
    try:
        ...
        if ambient_added and ambient_file:
            ...
            # O filtro volume aplica o ganho exato. O amix com normalize=0 não altera o volume.
...
async def synthesize(req: TTSRequest):
    ...
    try:
        ...
        # Atualiza estatísticas agregadas no dicionário compartilhado
